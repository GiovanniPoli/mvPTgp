...
if(FALSE){for(i in 2:6){
  test = metadata[ metadata$k %in% c(pfsr_data$k), ]
  cat(colnames(test)[i],":\n",
      rbind(names(table(test[,i])),table(test[,i]),"\n"))
  cat("\n")
}}
...
medians = vector(mode = "numeric",10000)

for(i in 1:10000){
  a = rPTmarg.med(correlated_levels = 6, truncation = 12, G0 = G0, G0inv = G0inv)
 medians[i] = a$tree[[12]]$Be[which.max( cumsum(a$G)>.5)]
}
quantile(medians, c(.025,.975))



table(metadata[metadata$k %in%unique(pfsr_data$k), "agent"])

# atezolizumab	- avelumab - durvalumab	- ipilimumab/nivolumab - nivolumab - pembrolizumab - pembrolizumab-or-nivolumab	
#Non-small-cell lung cancer 	2	0	1	2	0	1	0	  | 6
# Breast cancer 	            1	0	0	0	0	2	0	  | 3
# Melanoma 	                  2	2	0	0	2	0	0   | 6
# Other                     	2	5	5	0	4	11	1	|28
#### 
# TOT. Agent                	7	7	6	2	6	14	1	
...
PAR = GP_parameters.gen_par(B = B, Q = as.matrix(Qs), correlated_levels = D, star_model_deep = 12,
                           correlation_star_matrix =  K.star,
                           c     = c,
                           G0    = G0,
                           G0inv = G0inv,
                           n.cores = 50)
saveRDS(PAR,"Star_PFSr_02112023.rds")
...
Model.data = Meta_sampler.v1(data = pfsr_data,
               Qs = Qs[1:n_observed_study,],
               PRIORS = PRIORS,
               starting_values = starting_values,
               burn.in  = 1000,
               thinning = 25,
               sample   = 2500)
saveRDS(Model.data, file = "Model_02112023.rds")
...
for(obs in 1:length(Model.data$chain) ){
    
   cat(paste0("\r[",obs,"/",length(Model.data$chain),"]                         "))
   
   Zobs.0  = logit(Model.data$chain[[obs]]$Y0)
   Zobs.00 = logit(Model.data$chain[[obs]]$Ye0[,1])
   Zobs.10 = logit(Model.data$chain[[obs]]$Ye0[,2])
   ...
   Var_Z0.new     = S11.0  - S12.0  %*% S22inv.0  %*% S21.0
   Var_Z00.new    = S11.00 - S12.00 %*% S22inv.00 %*% S21.00
   Var_Z10.new    = S11.10 - S12.10 %*% S22inv.10 %*% S21.10 
   ...
   Ex_Yi0   = c(colMeans(expit(rmvnorm(10000, mean = c(Ex_Z0.new),  sigma = Var_Z0.new ))))
   Ex_Yi00  = c(colMeans(expit(rmvnorm(10000, mean = c(Ex_Z00.new), sigma = Var_Z00.new))))
   Ex_Yi10  = c(colMeans(expit(rmvnorm(10000, mean = c(Ex_Z10.new), sigma = Var_Z10.new))))
   ...
   Gs    = 1/2^(14-6) * Yie * Yiee        # after level 7 its used Ex as aprox (almost no variability)
   ...
   for(deep in 3:6){
     ...
     for(combination in 1:2^(deep-1)){
       ...
       Ye0 = expit(c(rmvnorm(1, mean  = PAR[[deep]][[combination]]$Ex[ new_index ],
                                sigma = PAR[[deep]][[combination]]$Var[ new_index, 
                                                                        new_index])))
       Ye1 = 1 - Ye0
       
       Ex_Ye0 = PAR[[deep]][[combination]]$Y_Ex[ new_index ]
       Ex_Ye1 = 1 - Ex_Ye0
       
       G_deep = cbind(G_deep, matrix(rep(Ye0,2^(14-deep)), ncol = 2^(14-deep), nrow = n_futures_study),
                              matrix(rep(Ye1,2^(14-deep)), ncol = 2^(14-deep), nrow = n_futures_study))
       
       Ex_G_deep = cbind(Ex_G_deep, matrix(rep(Ex_Ye0,2^(14-deep)), ncol = 2^(14-deep), nrow = n_futures_study),
                                    matrix(rep(Ex_Ye1,2^(14-deep)), ncol = 2^(14-deep), nrow = n_futures_study))
  ...
  G_new_medians    = sapply(1:32, function(row)  Pi_new[[14]][row, which.max(cumsum(   Gs[row, ])  > .5)])
  Ex_G_new_medians = sapply(1:32, function(row)  Pi_new[[14]][row, which.max(cumsum(Ex_Gs[row, ])  > .5)])
  

  positive = Gs[ seq(from = 1, to = 31, by = 2),] * W
  negative = Gs[ seq(from = 2, to = 32, by = 2),] * W
  
  Ex_positive = Ex_Gs[ seq(from = 1, to = 31, by = 2),] * W
  Ex_negative = Ex_Gs[ seq(from = 2, to = 32, by = 2),] * W
  
  m_p = Pi_new[[14]][1, which.max(cumsum(colSums(positive)) > .5)] 
  m_n = Pi_new[[14]][1, which.max(cumsum(colSums(negative)) > .5)] 
  
  Ex_m_p = Pi_new[[14]][1, which.max(cumsum(colSums(Ex_positive)) > .5)] 
  Ex_m_n = Pi_new[[14]][1, which.max(cumsum(colSums(Ex_negative)) > .5)] 
  
  Medians        = rbind(Medians,       G_new_medians)
  Ex_Medians     = rbind(Ex_Medians,    Ex_G_new_medians)
  
  Mixture.Median    = rbind(Mixture.Median,    c(m_p, m_n,       log(m_p/m_n)))
  Ex_Mixture.Median = rbind(Ex_Mixture.Median, c(Ex_m_p, Ex_m_n, log(Ex_m_p/Ex_m_n)))
  
}
 
p = colSums(Medians[, seq(from = 1, to = 31, by = 2)] > Medians[, seq(from = 2, to = 32, by = 2)]) / 2500
 
saveRDS(object =list( 
  "Ex_positive" = Ex_positive,
  ...
  "Ex_m_n" = Ex_m_n,
  "Medians"           = Medians,
  "Ex_Medians"        = Ex_Medians,
  "Mixture.Median"    = Mixture.Median,   
  "Ex_Mixture.Median" = Ex_Mixture.Median,
  "PostProb"       = p),
  file = "Post_real_data.rds")
